...
/**
 * nodes_count - Function that stores the number of nodes of heap tree
 * @root: Pointer to the head of the heap tree
 * Return: number of nodes in heap tree
 */
int nodes_count(heap_t *root)
{
	int l = 0, r = 0;
	...
	l = nodes_count(root->left);
	r = nodes_count(root->right);
